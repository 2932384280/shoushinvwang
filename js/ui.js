...
export function loadAvatarFromStorage() {
    try { const s = localStorage.getItem('queen_avatar_data'); if (s) { const d = JSON.parse(s); if (d?.type === 'custom' && d.url) { G.player.avatarData = d.url;
            G.player.avatar = 'custom'; return true; } else if (d?.type === 'emoji' && d.emoji) { G.player.avatar = d.emoji;
            G.player.avatarData = null; return true; } } } catch (e) {}
    return false;
}
export function saveAvatarToStorage(type, value) { try { const d = { type }; if (type === 'custom') d.url = value;
        else d.emoji = value;
        localStorage.setItem('queen_avatar_data', JSON.stringify(d)); } catch (e) { G.player.avatar = '👑';
        G.player.avatarData = null; } }
export function setAvatarEmoji(emoji) { G.player.avatar = emoji;
    G.player.avatarData = null;
    saveAvatarToStorage('emoji', emoji);
    renderTopbar(); }
export function setAvatarImage(base64) { if (!base64 || base64.length < 100) { setAvatarEmoji('👑'); return; }
    G.player.avatar = 'custom';
    G.player.avatarData = base64;
    saveAvatarToStorage('custom', base64);
    renderTopbar(); }
...
export function applyTheme(name) { const t = THEMES[name]; if (!t) return;
    G.currentTheme = name; const r = document.documentElement;
    r.style.setProperty('--gold', t.primary);
    r.style.setProperty('--gold-light', t.secondary);
    r.style.setProperty('--dark', t.bg);
    r.style.setProperty('--dark-card', t.bg + 'f0');
    localStorage.setItem('queen_theme', name); }
export function loadTheme() { const s = localStorage.getItem('queen_theme'); if (s && THEMES[s]) applyTheme(s); }

// ========== 设置相关 ==========
export function setAutoMode(mode) { G.autoSaveMode = mode;
    addLog(`🤖 自动存档：${mode}`);
    renderSettings(); }
export function setLogSyncMode(mode) { G.logSyncMode = mode;
    addLog(`📜 日志同步：${mode === 'all' ? '全部' : mode === 'none' ? '关闭' : '仅相关'}`, 'highlight');
    renderSettings(); }
...
export function renderStartScreen() {
    ...
    if (fi) fi.addEventListener('change', function(e) { const f = e.target.files[0]; if (!f) return; const r = new FileReader();
        r.onload = ev => { startAvatarType = 'custom';
            startAvatarData = ev.target.result;
            updateStartAvatarPreview();
            document.querySelectorAll('#startEmojiGrid .avatar-opt').forEach(o => o.classList.remove('selected')); };
        r.readAsDataURL(f);
        fi.value = ''; });
...
export function resetStartAvatar() { startAvatarType = '👑';
    startAvatarData = null;
    updateStartAvatarPreview();
    document.querySelectorAll('#startEmojiGrid .avatar-opt').forEach(o => { o.classList.remove('selected'); if (o.dataset.avatar == '👑') o.classList.add('selected'); }); }
...
export function startGame() {
    ...
    let at = startAvatarType,
        ad = startAvatarData;
    if (at === 'custom' && (!ad || ad.length < 100)) { at = '👑';
        ad = null; }
...
export function loadLastSave() { for (let i = 5; i >= 0; i--) if (loadFromSlot(i)) { document.getElementById('topbar').style.display = 'flex';
        document.getElementById('navbar').style.display = 'flex';
        renderAll();
        switchPage('queen');
        addLog(`📤 读取存档 ${i + 1}`, 'highlight'); return; }
    showModal('❌ 无存档', '没有找到可读取的存档。', [{ text: '知道了', action: closeModal }]); }

export function confirmRestart() { showModal('🔄 重新开始', '确定重新开始吗？', [{ text: '✅ 确认', action: () => { closeModal();
            restartGame(); } }, { text: '❌ 取消', action: closeModal }]); }
