import { pick, rand, clamp, SURNAMES, GIVEN_NAMES, RACE_DATA, RACE_KEYS, NATION_DATA, PLACE_DATA, getRaceData, getRaceEmoji, isMale, genderText, genderClass, wealthToCopper, formatWealth, addWealth, getDateString, timeStr } from './data.js';
...
export function generateCharacter(gender, race, nationId, ageMin, ageMax, familySurname) {
    ...
    if(ageMin===undefined) ageMin=16; if(ageMax===undefined) ageMax=60;
    ...
    return {
        ...
        combat, talent, level,
        inHarem:false, isLover:false, fertility:isMale(gender)?rand(30,80):rand(20,60),  // 雄性生育力
...
// ========== 存档系统（修复：包含hiddenCharacters，自动存档用slot 5） ==========
...
export function saveToSlot(idx) {
    ...
    try{
        const d = {
            ...
            hiddenCharacters: G.hiddenCharacters.map(c=>JSON.parse(JSON.stringify(c))), // 新增
            harem: [...G.harem],
            children: [...G.children],
...
export function loadFromSlot(idx) {
    ...
    try{
        ...
        G.hiddenCharacters = d.hiddenCharacters || []; // 新增
        G.harem = d.harem || [];
...
export function initGame(playerName,avatarType,avatarData,nationName,playerRace) {
    ...
    G.characters=[]; G.hiddenCharacters=[]; G.harem=[]; G.children=[]; G.exploredCount={}; G.logs=[]; G.gameStarted=true; G.gameActive=true;
    G._nextId=1; G._usedNames=new Set(); G.logSyncMode='all'; G.diplomacy={pending:null}; G.haremChambers=[]; G.achievements=[];
    ...
    if(avatarType==='custom'&&avatarData) saveAvatarToStorage('custom',avatarData); else saveAvatarToStorage('emoji',avatarType||'👑');
