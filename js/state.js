...
function buildInitialRelationships() {
    ...
    const raceGroups = {};
    all.forEach(c => { if (!raceGroups[c.race]) raceGroups[c.race] = [];
        raceGroups[c.race].push(c); });
    for (const race in raceGroups) {
        const list = raceGroups[race];
        if (list.length < 2) continue;
        const famMap = {};
        list.forEach(c => { const f = c.familySurname; if (!famMap[f]) famMap[f] = [];
            famMap[f].push(c); });
...
export function initNations(playerRace) {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        G.nations.push({
            ...
            name: d.name,
            race: d.race,
            raceEmoji: rd ? rd.emoji : '🏛️',
            desc: d.desc,
            environment: d.environment || '未知',
            power: rand(30, 80),
            wealth: rand(40, 90),
            military: rand(20, 70),
            relations: {},
    ...
    G.nations.forEach(n => {
        ...
        const ageMin = n.race === '人鱼族' ? 50 : 20,
            ageMax = n.race === '人鱼族' ? 200 : 50;
        const ruler = generateCharacter(rulerGender, n.race, n.id, ageMin, ageMax);
        ruler.met = true;
        ruler.isRuler = true;
        ruler.loyalty = 100;
        ruler.affection = 50;
        ruler.combat = rand(40, 90);
        ruler.talent = rand(20, 80);
    ...
    if (pn) {
        const addMinister = (gender, race, role, ageMin, ageMax, combatMin, combatMax, talentMin, talentMax) => {
            ...
            c.isMinister = true;
            c.role = role;
            c.loyalty = rand(75, 100);
            c.affection = rand(40, 60);
            c.combat = rand(combatMin, combatMax);
            c.talent = rand(talentMin, talentMax);
...
export function initGame(playerName, avatarType, avatarData, nationName, playerRace) {
    ...
    G.player.treasury = 60;
    G.player.military = 20;
    G.player.intelligence = 20;
    G.player.rule = 20;
    G.player.day = 1;
    G.player.month = 1;
    G.player.year = 360;
