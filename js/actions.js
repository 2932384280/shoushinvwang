import { G, getChar, getPlace, getNation, getPlayerNation, getNationPower, getNationDistance, addLog, addCharacterLog, generateCharacter, revealCharacter, unlockAchievement, autoSave, getAdultChildren, getDateString, timeStr } from './state.js';
import { pick, rand, clamp, RACE_DATA, isMale, genderText, wealthToCopper, formatWealth, addWealth } from './data.js';
...
export function advanceTime(steps) { /* 保留不使用 */ }
...
// 导出 completeDiplomacy 供其他模块使用
export function completeDiplomacy() {
    const pending = G.diplomacy.pending;
    if (!pending) return;
...
export function executeDiplomacy(action, target, messengerId) {
    ...
    if (action == 'marry') {
        ...
        G.player.prestige -= 20;
        target.relations[pn.id] = (target.relations[pn.id] || 0) + 25;
        pn.relations[target.id] = (pn.relations[target.id] || 0) + 25;
        addLog(`💍 与 ${target.name} 联姻成功！`, 'success');
        const spouse = generateCharacter('雄性', target.race, target.id, 20, 40);
        spouse.met = true;
        spouse.nation = target.id;
        G.characters.push(spouse);
    ...
    if (action == 'attack') {
        ...
        if (Math.random() > successRate) { showModal('⚔️ 攻打被拒', '战争爆发！', []); G.player.military = clamp(G.player.military - rand(10, 20), 0, 500);
            target.relations[pn.id] = (target.relations[pn.id] || 0) - 40; return; }
        const gain = rand(500, 3000);
        G.player.wealth = addWealth(G.player.wealth, gain);
        G.player.prestige = clamp(G.player.prestige + 10, 0, 200);
        target.military = Math.max(0, target.military - rand(5, 15));
        G.player.military = clamp(G.player.military - rand(3, 8), 0, 500);
    ...
    if (action == 'vassal') {
        ...
        target.vassal = pn.id;
        G.player.prestige = clamp(G.player.prestige + 15, 0, 200);
        target.relations[pn.id] = (target.relations[pn.id] || 0) - 10;
