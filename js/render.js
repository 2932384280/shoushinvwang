...
import { pick, rand, clamp, RACE_DATA, RACE_KEYS, isMale, genderText, genderClass, formatWealth, wealthToCopper, getDateString, timeStr } from './data.js';
...
export function switchPage(page) {
    ...
    const nb=document.querySelector(`.nav-item[data-page="${page}"]`); if(nb) nb.classList.add('active');
    switch(page){
...
export function renderQueen() {
    const p=G.player, pn=getPlayerNation(), rd=RACE_DATA[p.race];
    const stats = [
        {icon:'❤️',val:p.health+'/'+p.maxHealth,label:'生命'},{icon:'⚡',val:p.stamina+'/100',label:'体力'},
        {icon:'⭐',val:p.prestige,label:'威望'},{icon:'⚔️',val:p.military,label:'兵力'},{icon:'💖',val:p.charm,label:'魅力'},
        {icon:'🧠',val:p.intelligence,label:'智慧'},{icon:'👑',val:p.rule,label:'统治力'},{icon:'💰',val:formatWealth(p.wealth),label:'财富'},
...
export function renderPlaces() {
    ...
    const placeList = G.places.filter(p=>p.realm===realm).sort((a,b)=>{
        if(a.fixed=='first') return -1; if(b.fixed=='first') return 1;
        if(a.fixed=='last') return 1; if(b.fixed=='last') return -1;
...
export function setCharCategory(cat) { currentCharCategory=cat; renderCharacters(); }
...
export function renderSettings() {
    ...
    document.getElementById('content').innerHTML = `<div class="page active" id="page-settings"><div class="page-title">⚙️ 设置</div><div class="card"><div class="card-title">💾 存档管理</div><button class="btn btn-sm btn-block" onclick="openSaveLoad()">📂 存档 / 读档 (6个槽位)</button></div><div class="card"><div class="card-title">🏆 成就</div><div style="display:flex;flex-wrap:wrap;gap:2px;font-size:0.7em;">${achievementsHtml}</div></div><div class="card"><div class="card-title">🤖 自动存档</div><div style="display:flex;gap:3px;flex-wrap:wrap;">${modeBtns}</div></div><div class="card"><div class="card-title">📜 日志同步</div><div style="display:flex;gap:3px;flex-wrap:wrap;">${syncBtns}</div></div><div class="card"><div class="card-title">🎨 UI主题</div><div style="display:flex;flex-wrap:wrap;gap:2px;">${themeBtns}</div></div><div class="card"><button class="btn btn-sm btn-danger btn-block" onclick="confirmRestart()">🔄 重新开始</button></div></div>`;
    document.querySelectorAll('.color-dot').forEach(dot=> dot.addEventListener('click',function(){ applyTheme(this.dataset.theme); document.querySelectorAll('.color-dot').forEach(d=>d.classList.remove('active')); this.classList.add('active'); renderSettings(); }));
}
